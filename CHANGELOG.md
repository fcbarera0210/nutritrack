...
- **Campo de fecha en formulario de alimentos**: Agregado campo de fecha en el formulario de agregar alimentos, permitiendo registrar alimentos para fechas pasadas. Campo con validación (máximo fecha actual) y formato YYYY-MM-DD. El formulario ahora soporta modo edición y creación.
- **Corrección de timezone en hidratación**: Corregido problema de zona horaria en la card de hidratación del dashboard. Los horarios ahora se muestran correctamente según la zona horaria local del usuario. Nueva función `formatTimeLocal` en `lib/utils/date.ts` para formatear horas en formato HH:mm.
- **Tips informativos en modales**: Agregados textos informativos en los modales de comidas, ejercicios e hidratación explicando la funcionalidad de swipe. Diseño consistente con el tip del modal de cálculo de calorías de ejercicios, usando icono Lightbulb y texto simple para usuarios.
- **Mejoras visuales en cards swipeables**: Border-radius dinámico en cards cuando están deslizadas (sin border-radius en lado derecho cuando está abierta). Botones de acción con altura completa de la card. Border-radius de 15px en botón de eliminar (lado derecho). Ajuste de umbral de swipe para hidratación (35px) para facilitar la interacción con un solo botón.
- **Mejoras de TypeScript**: Eliminados imports no usados y reemplazados tipos `any` por tipos específicos en `dashboard/page.tsx`. Mejor tipado para ejercicios, entradas de agua y items de comida.
- **API Routes actualizadas**: `/api/logs/update` ahora acepta `servingSize` y `date` como campos opcionales para actualizar registros de alimentos.
